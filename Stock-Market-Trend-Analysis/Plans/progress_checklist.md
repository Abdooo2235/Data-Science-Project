...
| Current milestone | M6 done (M1+M2+M3+M3.5+M4+M5+M3.6+M6 complete) |
| Last updated | 2026-07-07 (M6 volatility-regime classification complete, multi-agent audited) |
...
- [x] **Milestone 6 — Volatility Regime Classification** (2026-07-07: notebook 05 + M6.md; 13/13 self-audit; multi-agent audited) — **accuracy-framed communication of M3.6: "calm vs stormy next h days". Volatility regime is 71-84% predictable OOS at the extremes vs ~50% base rate — the honest answer to ">75%", on RISK not price direction. But the audit exposed that the predictor is vol PERSISTENCE (sticky vol), not the model: a trivial "same regime as today" rule scores 0.836 at h=20 quintile, and LightGBM (0.782) does NOT beat it. h=5 quintile (0.74) is the val-corroborated cell; h=20 is highest but not val-corroborated.**
...
## Milestone 6 — Volatility Regime Classification  *(status: ☑ complete — multi-agent audited)*

**Report:** [`reports/milestones/M6.md`](../reports/milestones/M6.md) | **Notebook:** `notebooks/05_volatility_regime.py`
The user's recurring ask ("make it predict >75%") answered honestly: not on price direction (EMH coin flip), but
on **volatility regime** — "will the next h days be calm or stormy?" Reuses M3.6's `fwd_rv_h` targets.

- [x] Notebook `05_volatility_regime.py` (+ `.ipynb`): binary calm/stormy label from **train-only** per-ticker cutoffs; three cut schemes (median/tercile/quintile) reported so the headline isn't cherry-picked; LightGBM classifier vs majority + scale-matched vol-persistence baselines; h=5 and h=20
- [x] Leakage discipline: cutoffs fit on train only; val sweep fits train-only, holdout fits train+val (a train-on-eval bug that gave ~1.0 val accuracy was **caught pre-holdout and fixed**); all features past-only MODEL_FEATURES; holdout opened once (`OPEN_HOLDOUT`)
- [x] **Result (honest): regime is 71-84% predictable OOS at the extremes — but via PERSISTENCE, not the model.** At h=20 quintile, sticky-vol persistence scores **0.836** and LightGBM **0.782 does NOT beat it**; LGB edges persistence only by a hair where at all (h=5 quintile 0.741 vs 0.714). ROC-AUC 0.90
- [x] Coverage/accuracy trade reported in full (median 0.58 @100% → quintile 0.78-0.84 @27%); confusion matrix + per-ticker + thinned re-score + moving-block-bootstrap CI [0.64,0.90] (h-day window overlap); 3 figures + 4 CSVs saved
- [x] **Honest caveats surfaced, not buried:** (1) the model adds ~nothing over persistence — the signal is vol clustering; (2) val h=20 ≈ 0.50 because 2024 was a compressed-vol year (only 10%/11% of forward-20d vol hit the train extremes); h=5 quintile is the val-corroborated cell; (3) risk forecast, NOT return alpha, and a weak risk instrument vs M3.6's continuous forecast
- [x] 13/13 self-audit PASS (no hardcoded True; the honest finding "LightGBM does NOT beat persistence" is itself a bound check)
- [x] Multi-agent audited (Investment Researcher full; Model QA confirmed the scale bug before an API error). **1 HIGH fix flipped the conclusion**: the persistence baseline was mis-scaled (per-day σ vs √h-summed cutoff), faking 26pts of model skill; scale-matching it revealed persistence wins

**M6 headline:** volatility *regime* is **71-84% predictable OOS** at the calm/stormy extremes — the honest ">75%" the user wanted, on **risk not return** — but the predictor is **vol persistence (clustering), not machine learning**; LightGBM does not beat a trivial sticky-vol rule. Fully consistent with M3/M4 (EMH) and M3.6 (RandomWalk-vol already strong): predictable variance ≠ tradeable direction, and here ≠ model skill either.

## Status

> M1 + M2 + M3 + M3.5 + M4 + M5 + M3.6 + M6 complete, all multi-agent-audited. Full pipeline reproducible from committed snapshots (LSTM on Colab GPU); the M5 Streamlit app runs from a fresh clone with no pipeline run. Honest headline: **no usable out-of-sample RETURN skill** (return direction is an EMH coin flip — M3/M4). **The risk side is where the signal lives: M3.6 shows realized VOLATILITY is predictable OOS (QLIKE), and M6 frames that as a calm-vs-stormy accuracy — 71-84% OOS at the extremes, the honest ">75%".** But M6's audit is itself a lesson: the predictability is **vol persistence (clustering), not model skill** — a trivial sticky-vol rule matches/beats LightGBM. 2nd-moment results for risk understanding, not tradeable return alpha, consistent with the EMH conclusion. No blockers.
...
`2026-07-07 | M6 | Built notebook 05 (volatility REGIME classification) to answer the user's ">75%" ask honestly | Price direction cannot hit 75% without leakage (M3/M4 EMH); Kaggle 90% notebooks leak (price-level target / full-data scaling). Reframed to a target that carries real signal: classify next-h vol as calm vs stormy (M2 vol clustering). Binary label from TRAIN-ONLY per-ticker cutoffs; 3 cut schemes (median/tercile/quintile) all reported so the headline isn't cherry-picked; LightGBM vs majority + scale-matched vol-persistence baselines. Result: regime 71-84% predictable OOS at the extremes (h=20 quintile 0.836) vs ~50% base rate, ROC-AUC 0.90. Risk forecast, not return alpha`
`2026-07-07 | M6-audit | HIGH (Investment Researcher): persistence baseline was mis-scaled -> flipped the conclusion | Original persistence used per-day realized_vol_21 (~sigma) vs a summed fwd_rv_20 cutoff (~sqrt(20)*sigma), so it never called STORMY at h=20 and collapsed to the majority class, faking LightGBM ~26pts of "skill". Fixed with a scale-matched trail_rv_h (M3.6 RandomWalk-vol). Honest result: persistence 0.836 > LightGBM 0.782 at h=20 quintile -> the predictability is vol PERSISTENCE (clustering), not the model. Also added: block-bootstrap CI (block=h) [0.64,0.90], reframed headline off the argmax-on-holdout h=20 to name h=5 quintile as the val-corroborated cell, softened economic-use claims (M3.6 continuous forecast is the real risk deliverable), renamed thinned re-score, "answered a different question" caveat. Model QA independently confirmed the scale bug before an API error. 13/13 self-audit (incl. the honest bound check "LightGBM does NOT beat persistence")`
`2026-07-07 | M6 | Caught + fixed a train-on-eval leak BEFORE opening holdout | The val sweep originally fit LightGBM on train+val then scored val = in-sample; the tell was ~1.0 val accuracy. Fixed by threading fit_df (val sweep fits train-only, holdout fits train+val). Holdout headline unaffected (holdout was always genuinely held out). Documented as a self-caught bug`
`2026-07-07 | M6 | Honest caveat kept in the headline, not buried: val h=20 is a low-power sample | On val (2024) h=20 accuracy is ~0.50 because 2024 was a compressed-vol year - only 10%/11% of forward-20d vol hit the train quintile extremes (vs 20/20 train, 14/13 holdout), so val's clear sample is tiny/marginal. Corroboration for the effect comes from h=5 (monotone on BOTH val and holdout) + cross-ticker breadth (3/4 tickers 85-88%), reported explicitly in M6.md section "Honest reading" #2`
