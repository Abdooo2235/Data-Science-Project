...
- **M3.6** ✅ — Volatility forecasting (extra): realized vol *is* predictable out-of-sample (unlike return direction). Notebook: `notebooks/04_volatility_modeling.py`. Report: [reports/milestones/M3.6.md](reports/milestones/M3.6.md).
- **M6** ✅ — Volatility **regime** classification (extra): "calm vs stormy next h days" — **71–84% out-of-sample accuracy** at the extremes, the honest answer to ">75%" (a *risk* forecast, not price direction). The audit's lesson: that predictability is **vol persistence, not model skill** — a trivial sticky-vol rule matches/beats LightGBM. Notebook: `notebooks/05_volatility_regime.py`. Report: [reports/milestones/M6.md](reports/milestones/M6.md).
